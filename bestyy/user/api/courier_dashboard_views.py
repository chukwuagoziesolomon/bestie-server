...
from user.models import CourierProfile, Order, DailyStats
from .serializers import DeliverySerializer, DailyStatsSerializer
...
def earnings_chart_data(request):
    ...
    while current_date <= end_date:
        try:
            ...
            stats = DailyStats.objects.get(courier=courier, date=current_date)
        except DailyStats.DoesNotExist:
            # Create stats for the day
            stats = DailyStats.update_stats_for_date(courier, current_date)
        ...
        daily_stats.append({
            ...
            'avg_time': stats.avg_delivery_time
        })
...
def update_delivery_status(request, order_id):
    ...
    # Update daily stats if order is delivered
    if new_status == 'delivered' and order.delivered_at:
        DailyStats.update_stats_for_date(user.courier_profile, order.delivered_at.date())
