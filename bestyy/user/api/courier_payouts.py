...
from user.models import Order, DailyStats
...
class CourierEarningsBreakdownView(APIView):
    ...
    def get(self, request):
        ...
        weekly_breakdown = self._calculate_weekly_breakdown(daily_stats, start_date, end_date)
        
        # Calculate monthly summary
        monthly_summary = self._calculate_monthly_summary(daily_stats)
        
        # Get year-to-date summary
        ytd_start = date(year, 1, 1)
        ytd_stats = DailyStats.objects.filter(
            courier=courier,
            date__range=(ytd_start, end_date)
        )
        ytd_summary = self._calculate_ytd_summary(ytd_stats)
    ...
    def _calculate_weekly_breakdown(self, daily_stats, start_date, end_date):
        ...
        while current_date <= end_date:
            ...
            week_stats = daily_stats.filter(date__range=(week_start, week_end))
            
            week_earnings = sum(stat.total_earnings for stat in week_stats)
            week_deliveries = sum(stat.total_deliveries for stat in week_stats)
    ...
    def _calculate_monthly_summary(self, daily_stats):
        ...
        total_earnings = sum(stat.total_earnings for stat in daily_stats)
        total_deliveries = sum(stat.total_deliveries for stat in daily_stats)
        
        # Find best and worst days
        best_day = max(daily_stats, key=lambda x: x.total_earnings)
        worst_day = min(daily_stats, key=lambda x: x.total_earnings)
        
        # Count working days (days with earnings)
        working_days = sum(1 for stat in daily_stats if stat.total_earnings > 0)
        
        return {
            'total_earnings': float(total_earnings),
            'total_deliveries': total_deliveries,
            'average_earnings_per_day': round(float(total_earnings) / len(daily_stats), 2),
            'best_day': {
                'date': best_day.date.isoformat(),
                'earnings': float(best_day.total_earnings),
                'deliveries': best_day.total_deliveries
            },
            'worst_day': {
                'date': worst_day.date.isoformat(),
                'earnings': float(worst_day.total_earnings),
                'deliveries': worst_day.total_deliveries
            },
            'total_working_days': working_days,
            'total_calendar_days': len(daily_stats)
        }
    
    def _calculate_ytd_summary(self, ytd_stats):
        ...
        total_earnings = sum(stat.total_earnings for stat in ytd_stats)
        total_deliveries = sum(stat.total_deliveries for stat in ytd_stats)
        
        # Calculate months elapsed
        first_date = min(stat.date for stat in ytd_stats)
        last_date = max(stat.date for stat in ytd_stats)
        months_elapsed = (last_date.year - first_date.year) * 12 + last_date.month - first_date.month + 1
