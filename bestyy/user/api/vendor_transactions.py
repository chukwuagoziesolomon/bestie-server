...
from ..models import Order, VendorProfile, VendorAnalytics
...
def vendor_transaction_analytics(request):
    ...
    # Get top performing days of the week using VendorAnalytics model
    # Get analytics for current month
    current_month_start = timezone.now().replace(day=1)
    day_performance = VendorAnalytics.objects.filter(
        vendor=vendor,
