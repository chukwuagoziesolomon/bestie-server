...
from .models import Order, Payment, VendorProfile, UserProfile, CourierProfile, User, VendorAnalytics
...
def order_created_handler(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        # Update vendor analytics for the order date
        try:
            VendorAnalytics.update_analytics_for_date(instance.vendor, instance.created_at.date())
        except Exception as e:
            logger.error(f"Error updating vendor analytics for order {instance.id}: {str(e)}")
...
def payment_created_handler(sender, instance, created, **kwargs):
    """Send real-time update when new payment is created"""
    if created:
        activity_data = {
            'id': f"payment_{instance.id}",
            'type': 'payment',
            'title': f"Payment {instance.status.title()}",
            'description': f"{instance.user.get_full_name() or instance.user.username} made a payment of ₦{instance.amount}",
            ...
            'timestamp': instance.created_at.isoformat(),
            'icon': 'credit-card',
            'color': get_payment_status_color(instance.status)
        }
        
        send_activity_update(activity_data)
        print(f"🔥 REAL-TIME UPDATE: New payment {instance.paystack_reference} sent to admin dashboard")
