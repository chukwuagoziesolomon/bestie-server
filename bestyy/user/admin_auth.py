...
class CustomAdminSite(admin.AdminSite):
    ...
    def get_app_list(self, request):
        """
        Add custom views to the admin index.
        """
        ...
        custom_views = []
        for view_info in self._custom_views:
            if view_info['visible']:
                custom_views.append({
                    'name': view_info['label'],
                    'object_name': view_info['name'],
                    'admin_url': f'admin:{view_info["name"]}',
                    'view_only': True,
                    'icon_class': view_info['icon_class']
                })
        
        if custom_views:
            app_list.append({
                'name': 'Custom Views',
                'app_label': 'custom_views',
                'app_url': '#',
